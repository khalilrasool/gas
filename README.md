...
3. Set the `SPREADSHEET_ID` and `SHEET_NAME` constants in `Code.gs` to match your sheet (the script will remind you if the ID is missing).
...
5. Images for the personal photo and ID card are fetched and embedded as data URLs so they render correctly in the PDF. The script batches external image downloads to reduce execution time; use sharing links from Drive or direct URLs in the `img` and `card_img` columns.
